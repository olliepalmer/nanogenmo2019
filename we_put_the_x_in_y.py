...
total = file_len('corpora/words_alpha.txt')
print (str(total)+' total words')

counter = 0
with open('corpora/words_alpha.txt','r') as a:
    for wordtofind in a:
        if (len(str.rstrip(wordtofind)) > 1):
            with open('corpora/words_alpha.txt','r') as b:
                for wordtosearch in b:
                    if (f(wordtosearch,wordtofind) != []) and (wordtosearch != wordtofind) and (len(wordtofind) > 1):
                        ...
                        with open('corpora/we_put_the_x_in_y_2019_11_30.txt','a') as out:
                            out.write(text)
        counter += 1
